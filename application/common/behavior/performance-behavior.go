...
func (p *PerformanceBehavior) Execute(ctx context.Context, command mediator.Command, next mediator.CommandHandler) (interface{}, error) {
	defer func(start time.Time) {
		if time.Since(start).Milliseconds() > 500 {
			p.logger.Println(fmt.Sprintf("Performance Warning Log Command:%v, CorrelationId: %v, Duration(ms): %v", ctx.Value(common.CorrelationId), command, time.Since(start).Milliseconds()))
		}
	}(time.Now())
